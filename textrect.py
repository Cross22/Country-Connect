...
class TextRectException:
    def __init__(self, word, string, problem, message = None):
...
def render_textrect(string, font, rect, text_color, background_color, justification=0):
        ...
        if font.size(requested_line)[0] > rect.width:
            ...
            for word in words:
                if font.size(word)[0] >= rect.width:
                    raise TextRectException(word, string, 'too long', "The word " + word + " is too long to fit in the rect passed.")
            # Start a new line
            accumulated_line = ""
            for word in words:
    ...
    for line in final_lines:
        if accumulated_height + font.size(line)[1] >= rect.height:
            raise TextRectException(word, string, 'too tall', "Once word-wrapped, the text string was too tall to fit in the rect.")
        if line != "":
            ...
            if justification == 0:
                ...
                raise TextRectException(word, string, 'justification', "Invalid justification argument: " + str(justification))
